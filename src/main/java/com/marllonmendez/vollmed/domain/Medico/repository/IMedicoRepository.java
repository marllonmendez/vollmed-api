...
public interface IMedicoRepository extends JpaRepository<Medico, Long> {
    ...
    @Query("select m from Medicos m where m.disponivel = true and m.ramo = :ramo and m.id not in (select c.medico.id from Consultas c where c.data = :data) order by function('random') limit 1")
